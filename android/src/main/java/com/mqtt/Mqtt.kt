...
class Mqtt(reactContext: ReactApplicationContext, originId:String) {
  ...
  fun connect(host: String?, options: ReadableMap, callback: Callback) {
    ...
    try  {
      ...
      if (options.hasKey("tls")) {
        ...
        var trustManagers: Array<javax.net.ssl.TrustManager?>? = null
        if (cert != null && key != null) {
          val keyManagerFactory: javax.net.ssl.KeyManagerFactory = javax.net.ssl.KeyManagerFactory.getInstance("PKIX")
          val keyStore: java.security.KeyStore = EnvKeyStore.createFromPEMStrings(key, cert, "").keyStore()
          keyManagerFactory.init(keyStore, "".toCharArray())
          keyManagers = keyManagerFactory.keyManagers
        }
        if (ca != null) {
          val cf: java.security.cert.CertificateFactory = java.security.cert.CertificateFactory.getInstance("X.509")
          val caInput: java.io.InputStream = java.io.ByteArrayInputStream(Base64.decode(ca, Base64.DEFAULT))
          val caCert: Certificate = cf.generateCertificate(caInput)
          caInput.close()
          val keyStore: java.security.KeyStore = java.security.KeyStore.getInstance(java.security.KeyStore.getDefaultType())
          keyStore.load(null, null)
          keyStore.setCertificateEntry("ca", caCert)
          val tmf: javax.net.ssl.TrustManagerFactory = javax.net.ssl.TrustManagerFactory.getInstance(javax.net.ssl.TrustManagerFactory.getDefaultAlgorithm())
          tmf.init(keyStore)
          trustManagers = tmf.trustManagers
        }
        if (keyManagers != null || trustManagers != null) {
          val sslContext: javax.net.ssl.SSLContext = javax.net.ssl.SSLContext.getInstance("TLS")
          sslContext.init(keyManagers, trustManagers, java.security.SecureRandom())
          connOpts.setSocketFactory(sslContext.socketFactory)
        }
      }
  ...
  companion object {
    ...
    fun sendEvent(eventName: String, params: WritableMap) {
      ...
      _reactContext?.getJSModule(RCTNativeAppEventEmitter::class.java)
